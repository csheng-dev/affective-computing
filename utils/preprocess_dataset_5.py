...
# -*- coding: utf-8 -*-
"""
Created on Fri Sep  5 16:09:44 2025
...
def preprocess_data_5(fs, chunk_duration, overlap_duration, root_path):
    
    base_path = root_path + '/5/MEFAR/'
    ...
    for i in range(len(subject_dirs)):
        ...
        for j in range(len(subject_subdirs)):
            ...
            # path on mac            
            no = subject_dirs[i].replace("_", "")
            # torch.save(chunks, f'/Users/sheng/Documents/emotion_model_project/preprocessed_data/dataset_5_chunk_{subject_subdirs[j]}_{no}.pt') 
            # path on server
            torch.save(chunks, f'/home/sheng/project/affective-computing/preprocessed_data/dataset_5_chunk_{subject_subdirs[j]}_{no}.pt')
            
        
preprocess_data_5(fs, chunk_duration, overlap_duration, root_path) 
